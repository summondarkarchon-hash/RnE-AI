...
log.info('데이터 로드 시작...')
df_full   = _load('rne_full.csv').reset_index(drop=True)      # 전체 RnE 1,357건 — 유사 주제 검색
df_collab = _load('rne_collab.csv').reset_index(drop=True)    # 대학 협업 RnE 171건 — 기관 스코어링
df_equip  = _load('dgb_equipment.csv').reset_index(drop=True)
df_rnd    = _load('dgb_research.csv').reset_index(drop=True)
